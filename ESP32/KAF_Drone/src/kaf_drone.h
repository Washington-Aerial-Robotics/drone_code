...
typedef union coordinate {
  struct {
    ...
    float y;
    float z;
    float stdev; // should this maybe be variance, since that is used more often in filtering?
  };
  float f[4];
};
...
typedef PACKED struct state {
  ...
  coordinate t;//orientation (rpy? or euler angles?)
  float[4] q;  //attitude quaternion w x y z
